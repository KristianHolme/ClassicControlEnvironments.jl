using ClassicControlEnvironments: reward
...
function _force_arrow_coords(problem::CartPoleProblem)
    ...
    return (; cart_pos, dx, dy, color, force)
end
...
function ClassicControlEnvironments.plot(problem::CartPoleProblem)
    ...
    if abs(problem.force) > 1.0e-3
        arrow_data = _force_arrow_coords(problem)
        arrows2d!(
            ax, [arrow_data.cart_pos], [Vec2f(arrow_data.dx, arrow_data.dy)],
...
function ClassicControlEnvironments.interactive_viz(env::CartPoleEnv)
    ...
    total_rew = Observable(0)
    auto_running = Observable(false)
    current_action = Observable(0)  # Track the latest action (0 = left, 1 = right)
    ...
    ax.xlabel = "Position"
    ax.ylabel = "Height"
    ax.title = "CartPole Environment - Use ← → arrow keys or buttons"
    xlims!(ax, track_left, track_right)
    ylims!(ax, -0.3, 1.2)
    ...
    autoplay_button = Button(button_grid[1, 3], label = "Autoplay", tellwidth = false)
    stop_button = Button(button_grid[1, 4], label = "Stop", tellwidth = false)
    reset_button = Button(button_grid[1, 5], label = "Reset", tellwidth = false)

    # Accumulated time for tick-based auto-stepping
    accumulated_time = Ref(0.0)

    # Function to perform an action and update observables
    ...
    on(left_button.clicks) do n
        if !auto_running[]
            perform_action(0)  # Push left
        end
    end

    on(right_button.clicks) do n
        if !auto_running[]
            perform_action(1)  # Push right
        end
    end

    # Autoplay button - toggle auto-running state
    on(autoplay_button.clicks) do n
        if !auto_running[]
            auto_running[] = true
            autoplay_button.buttoncolor = :lightgreen
            autoplay_button.label = "Autoplay (Running)"
    ...
    on(stop_button.clicks) do n
        if auto_running[]
            auto_running[] = false
            autoplay_button.buttoncolor = :lightgray
    ...
    on(reset_button.clicks) do n
        if !auto_running[]
            ...
            rew[] = reward(env)
            total_rew[] = rew[]
            current_action[] = 0  # Reset to default action
    ...
    on(events(fig).keyboardbutton) do event
        if event.action == Keyboard.press || event.action == Keyboard.repeat
            if !auto_running[]
                ...
                # During autoplay, keyboard can change the current action
                if event.key == Keyboard.left
                    current_action[] = 0
                elseif event.key == Keyboard.right
                    current_action[] = 1
    ...
    return x, theta, force, fig, button_grid, left_button, right_button, autoplay_button, stop_button, reset_button
end
...
function ClassicControlEnvironments.plot_trajectory(env::CartPoleEnv, observations::AbstractArray, actions::AbstractArray, rewards::AbstractArray)
    ...
    hlines!(
        ax_angle, rad2deg.([-env.problem.theta_threshold_radians, env.problem.theta_threshold_radians]),
...
function ClassicControlEnvironments.plot_trajectory_interactive(env::CartPoleEnv, observations::AbstractArray, actions::AbstractArray, rewards::AbstractArray)
    ...
    on(events(fig).tick) do tick
        if play_toggle.active[]
            ...
            step_interval = env.problem.tau / speed
