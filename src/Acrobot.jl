...
function DrillInterface.reset!(env::AcrobotEnv)
...
function DrillInterface.act!(env::AcrobotEnv, action::AbstractArray{Int, 1})
    return DrillInterface.act!(env, action[1])
end

function DrillInterface.act!(env::AcrobotEnv, action::Integer)
    problem = env.problem
    ...
    problem.theta1 = wrap_angle(new_state[1], -Float32(π), Float32(π))
    problem.theta2 = wrap_angle(new_state[2], -Float32(π), Float32(π))
...
function DrillInterface.observe(env::AcrobotEnv)
    p = env.problem
    # Return [cos(θ1), sin(θ1), cos(θ2), sin(θ2), θ̇1, θ̇2]
    return [cos(p.theta1), sin(p.theta1), cos(p.theta2), sin(p.theta2), p.dtheta1, p.dtheta2]
end

function DrillInterface.terminated(env::AcrobotEnv)
    p = env.problem
    # Goal condition: -cos(θ1) - cos(θ2 + θ1) > 1.0
    return -cos(p.theta1) - cos(p.theta2 + p.theta1) > 1.0f0
end

DrillInterface.truncated(env::AcrobotEnv) = env.step >= env.max_steps
DrillInterface.action_space(env::AcrobotEnv) = env.action_space
DrillInterface.observation_space(env::AcrobotEnv) = env.observation_space
DrillInterface.get_info(env::AcrobotEnv) = Dict{String, Any}(
    "step" => env.step,
